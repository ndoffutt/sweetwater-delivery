 ...
 *  which accounts are getting the attention. Shared by the manager's own
 *  route and the owner's ops-shell wrapper — see app/(ops)/reports/prospects. */
export default async function ReportsProspectsPage({
  links = DISPATCH_LINKS,
}: {
  links?: ProspectReportLinks;
} = {}) {
  const session = await getSession();
  if (!session) redirect("/");
  // Reports are for the office (owner + manager). Drivers can't reach it,
  // even by typing the URL — same gate as /dispatch/reports.
  ...
  return (
    <div className="mx-auto max-w-[1440px] px-5 md:px-12 py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Band title="Most-worked accounts" right={<Link href={links.touchpoints} className="font-barlowc text-[12px] uppercase tracking-[0.08em] text-ops-accent">Full log</Link>}>
          {byProspect.length === 0 ? (
        ...
        <Band title="Check-ins due" right={<Link href={links.checkins} className="font-barlowc text-[12px] uppercase tracking-[0.08em] text-ops-accent">Open</Link>}>
          {due.length === 0 ? (
